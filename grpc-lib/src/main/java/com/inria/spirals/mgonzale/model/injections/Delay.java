...
public class Delay extends Injection
{
	 ...
	 private static final Logger LOG = LoggerFactory.getLogger(Delay.class);
	    ...
	    public Delay(final Faultinjection.InjectionAction action, final long amount, final int sleep) {
	        super(Faultinjection.InjectionType.DELAY, action);
	        this.amount = amount;
	        this.sleep = sleep;
	    }
	 ...
	 @Override
	    public Injection setAction(final Faultinjection.InjectionAction action) {
	        return new Delay(action, this.amount, this.sleep);
	    }
	    
	    @Override
	    public String toString() {
	        return "Delay{amount=" + this.amount +", sleep="+this.sleep + "} " + super.toString();
	    }
